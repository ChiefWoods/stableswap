...
mod tests {
    ...
    fn swap_a_to_b() {
        ...
        let swap_user_a_ata =
            create_ata_for_owner(&mut ctx, &actors.swap_user, &initialized.mint_a, &actors.swap_user);
        let swap_user_b_ata =
            create_ata_for_owner(&mut ctx, &actors.swap_user, &initialized.mint_b, &actors.swap_user);
        ...
        let vault_a_before: TokenAccount = get_spl_account(&ctx.svm, &initialized.mint_a_vault).unwrap();
        let vault_b_before: TokenAccount = get_spl_account(&ctx.svm, &initialized.mint_b_vault).unwrap();

        let mut swap_ix = ctx
            .program()
            .accounts(stableswap::accounts::Swap {
                pool: initialized.pool,
                user: actors.swap_user.pubkey(),
                token_program: Token::id(),
            })
        ...
        ctx.execute_instruction(swap_ix, &[&actors.swap_user]).unwrap();
